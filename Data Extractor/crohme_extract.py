...
"""
@author: cheri
Modified from ThomasLech/CROHME_extractor/extract.py
Changes: 
    arguments (added dataset_path & output_path), 
    removed saving for classes.txt, 
    pickle features is orginal image shape and label is actual class,
    added exception in traininginkml loop
    
Use like:
    !python crohme_extract.py -b 45 -d 2011 2012 2013 -c all -t 1 -p /content/drive/MyDrive/TC11_package/ -o /content/drive/MyDrive/Output
"""
...
ap.add_argument('-p', '--dataset_path', required=True, help="Specify what dataset path have to be extracted.")
ap.add_argument('-o', '--output_path', required=True, help="Specify the output path to save.")
...
# Extract INKML files
data_dir = os.path.normpath(args.get('dataset_path'))
...
# Dump extracted data
outputs_dir = os.path.normpath(args.get('output_path'))

# Make directories if needed
if not os.path.exists(outputs_dir):
    os.mkdir(outputs_dir)

with open(os.path.join(outputs_dir, 'train.pickle'), 'wb') as f:
    pickle.dump(train, f, protocol=pickle.HIGHEST_PROTOCOL)
    print('Training data has been successfully dumped into', f.name)
with open(os.path.join(outputs_dir, 'test.pickle'), 'wb') as f:
